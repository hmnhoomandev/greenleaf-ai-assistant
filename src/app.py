...
from src.brain import filter_by_role, respond, translate_text, validate_role
from src.it_security_handler import is_it_security_query
from src.privacy_gate import clean_input, get_block_message, is_blocked
...
def process_query(raw_query, say, client, channel, user_id):
    ...
    if debug_level in ("compact", "extended"):
        raw_query = (
            raw_query.replace("--debug/extended", "")
            .replace("--debug/compact", "")
            .strip()
        )
    ...
    try:
        initial_response = client.chat_postMessage(channel=channel, text=wait_text)
    ...
    try:
        ...
        if secure_id in conversation_state:
            ...
            if debug_level is None:
                debug_level = (
                    state.get("debug_level") if isinstance(state, dict) else None
                )
            ...
            t_role = time.time()
            role_confirmed = fuzzy_match_role(query_in_english) or validate_role(
                query_in_english
            )
            ...
            print(
                f"[APP] Follow-up query_handbook: {round(time.time() - t_handbook, 2)}s"
            )
            ...
            print(
                f"[APP] Follow-up filter_by_role: {round(time.time() - t_filter, 2)}s"
            )
        ...
        # Step 4 — brain router
        result, tool_used, intent = respond(
            query_in_english, user_lang, user_id=secure_id
        )
